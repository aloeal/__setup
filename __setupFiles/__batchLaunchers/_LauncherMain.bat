@echo off
:: -- NIST DCS Combs Controller Software --

:: This bat file launches DCS Combs Controller software
:: - @ECHO lines do not print to console, only use rem within loops and :: outside loops for comments

:: NEW Setup: 
:: change PATH_ on line 15 in "" to path where >sysCode\DCSCombControl\ is located on PC getting eBUS
...
:: alt path to repo
rem set "PATH_=C:\Users\fcomb\OTTRepos\DCSCombControl\"
...
if not exist !SETUP!activate.bat ( echo ERROR: Virtual environment not found^! Ensure setup.bat build venv in dcs. & pause && exit /b )
